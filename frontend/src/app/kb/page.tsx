"use client";

import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Input } from "@/components/ui/input";
import { Separator } from "@/components/ui/separator";
import { Textarea } from "@/components/ui/textarea";
import { API_URL } from "@/lib/config";
import { FormEvent, useState } from "react";

/* ================= TYPES ================= */

type Source = {
  source: string;
  chunkId: number;
};

type AskResponse = {
  answer: string;
  sources: Source[];
  confidence: number;
};

type IngestResult = {
  ok: boolean;
  docCount: number;
  chunkCount: number;
  source: string;
};

/* ================= COMPONENT ================= */

export default function LightRagKB() {
  const [ingestText, setIngestText] = useState("");
  const [ingestSource, setIngestSource] = useState("");
  const [ingestLoading, setIngestLoading] = useState(false);
  const [ingestMsg, setIngestMsg] = useState<string | null>(null);

  const [question, setQuestion] = useState("");
  const [topK, setTopK] = useState(2);
  const [askLoading, setAskLoading] = useState(false);
  const [time, setTime] = useState<number | null>(null);

  const [answerData, setAnswerData] = useState<AskResponse | null>(null);
  const [showSources] = useState(true);

  /* ================= INGEST ================= */

  async function handleIngest(event: FormEvent) {
    event.preventDefault();
    setIngestLoading(true);
    setIngestMsg(null);

    try {
      const res = await fetch(`${API_URL}/kb/ingest`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          text: ingestText,
          source: ingestSource || undefined,
        }),
      });

      const json: IngestResult | { error: string } = await res.json();

      if (!res.ok) throw new Error("Ingest failed");

      const result = json as IngestResult;
      setIngestMsg(`Ingested ${result.chunkCount} chunks from ${result.source}`);
    } catch (e) {
      console.error(e);
    } finally {
      setIngestLoading(false);
    }
  }

  /* ================= ASK ================= */

  async function handleAskQuerySubmit(event: FormEvent) {
    event.preventDefault();
    setAskLoading(true);
    setAnswerData(null);
    setTime(null);

    const start = performance.now();

    try {
      const res = await fetch(`${API_URL}/kb/ask`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          query: question,
          k: topK,
        }),
      });

      const json: AskResponse | { error: string } = await res.json();
      if (!res.ok) throw new Error("Ask failed");

      setAnswerData(json as AskResponse);
    } catch (e) {
      console.error(e);
    } finally {
      setTime(Math.round(performance.now() - start));
      setAskLoading(false);
    }
  }

  /* ================= UI ================= */

  return (
    <div className="mx-auto max-w-5xl px-4 py-8 flex flex-col gap-8">
      <header className="flex flex-col gap-1">
        <h1 className="text-2xl font-bold tracking-tight">
          Knowledge Base (KB)
        </h1>
        <p className="text-sm text-muted-foreground">
          Light RAG Demo. Add your own docs, then ask questions. The model answers
          only from ingested content.
        </p>
      </header>

      <section className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* ===== INGEST ===== */}
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base font-semibold">
              Add to KB
            </CardTitle>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleIngest} className="flex flex-col gap-4">
              <div className="flex flex-col gap-2">
                <label className="text-xs font-medium text-muted-foreground">
                  Source Label
                </label>
                <Input
                  placeholder="Add your source here..."
                  value={ingestSource}
                  onChange={(e) => setIngestSource(e.target.value)}
                />
              </div>

              <div className="flex flex-col gap-2">
                <label className="text-xs font-medium text-muted-foreground">
                  Text / Markdown
                </label>
                <Textarea
                  value={ingestText}
                  onChange={(e) => setIngestText(e.target.value)}
                  className="min-h-[200px] font-mono text-xs resize-y"
                  placeholder="Paste docs, policies, or onboarding notes..."
                />
              </div>

              <div className="flex gap-2">
                <Button
                  type="button"
                  variant="secondary"
                  onClick={() => {
                    setIngestText("");
                    setIngestSource("");
                    setIngestMsg(null);
                  }}
                >
                  Reset
                </Button>
                <Button type="submit">
                  {ingestLoading ? "Ingesting..." : "Ingest to KB"}
                </Button>
              </div>
            </form>

            {ingestMsg && (
              <div className="mt-2 text-xs text-green-600">{ingestMsg}</div>
            )}
          </CardContent>
        </Card>

        {/* ===== ASK ===== */}
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base font-semibold">
              Ask the KB
            </CardTitle>
          </CardHeader>
          <CardContent className="flex flex-col gap-5">
            <form onSubmit={handleAskQuerySubmit} className="flex flex-col gap-4">
              <div className="flex flex-col gap-2">
                <label className="text-xs font-medium text-muted-foreground">
                  Your Query
                </label>
                <Input
                  placeholder="Ask a question..."
                  value={question}
                  onChange={(e) => setQuestion(e.target.value)}
                />
              </div>

              <div className="flex flex-col gap-2">
                <label className="text-xs font-medium text-muted-foreground">
                  Top K Chunks
                </label>
                <Input
                  type="number"
                  min={1}
                  max={5}
                  value={topK}
                  onChange={(e) =>
                    setTopK(parseInt(e.target.value || "2", 10))
                  }
                />
              </div>

              <Button type="submit">
                {askLoading ? "Asking..." : "Ask KB"}
              </Button>
            </form>

            {answerData && (
              <div className="flex flex-col gap-4 pt-5">
                <div className="rounded-md border bg-muted/30 p-3 text-sm whitespace-pre-wrap">
                  {answerData.answer}
                </div>

                <div className="text-sm text-green-600">
                  Time: {time} ms
                </div>

                {showSources && (
                  <div className="flex flex-col gap-2">
                    <span className="text-sm font-medium">
                      Sources ({answerData.sources.length})
                    </span>
                    <Separator />
                    <ul className="space-y-2">
                      {answerData.sources.map((s, i) => (
                        <li key={i} className="text-xs">
                          {s.source} <span>#{s.chunkId}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
                )}
              </div>
            )}
          </CardContent>
        </Card>
      </section>
    </div>
  );
}
